...
impl Organizer {
    ...
    pub async fn organize_file(&self, file_info: &FileInfo) -> Result<OperationResult> {
        let dest_path = self.determine_destination(file_info).await?;

        debug!("Organizing {:?} -> {:?}", file_info.path, dest_path);

        if self.dry_run {
            info!(
                "[DRY RUN] Would {} {:?} to {:?}",
                self.mode, file_info.path, dest_path
            );
        ...
        match result {
            Ok(()) => {
                if self.preserve_timestamps {
                    self.preserve_timestamps_for(&file_info.path, &dest_path)
                        .await?;
                }
    ...
    pub async fn determine_destination(&self, file_info: &FileInfo) -> Result<PathBuf> {
        let date = file_info
            .metadata
            .date_taken
            .as_ref()
        ...
        let subdir = match &self.pattern {
            ...
            OrganizationPattern::Type => file_info.file_type.category().to_string(),
